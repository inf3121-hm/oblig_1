...
private String getRandWord() { //Gets random word from word list
		Random rand = new Random();
		String randWord = wordForGuesing[rand.nextInt(9)];
		return randWord;
	}
	
public void displayMenu() { //displays menu and starts the game
		System.out
...
private void findLetterAndPrintIt() {
		...
		do {//debuging mode, shows the word
			System.out.println("The secret word is: " + printDashes(dashBuff));
			System.out.println("DEBUG " + guesWord);
			
			do { //starts scanner to scan for input and match it with the word
				System.out.println("Enter your gues(1 letter alowed): "); 
				Scanner input = new Scanner(System.in);
				letter = input.next();

				if (letter.equals(Command.help.toString())) { //checks if the input is equal to the command help
					isHelpUsed = true;
					int i = 0, j = 0;
					while (j < 1) {
						if (dashBuff.charAt(i) == '_') { //replaces _ with a correct letter
			...
			if (counter == 0) {//shows the mistakes the user has made
				++mistakes;
				{
					System.out.printf(
		...
		if (isHelpUsed == false) { //if the user has not used the command help, it shows the score and mistakes
		...
		} else { //user has cheated ie used the command help, doesnt allow the user to input a highscore
			System.out
